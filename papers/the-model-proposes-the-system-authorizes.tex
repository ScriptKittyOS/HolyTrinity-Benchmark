...
Every empirical claim in this paper about the system is a conformance claim, an explicitly labeled absence, or a result reported in the separately published evaluation~\cite{croft2026eval} and cited as such; this paper contains no security measurement of its own. The AgentBound differentiation in Section~\ref{sec:related-agentbound} rests on the published description rather than the source and should be re-verified; note also that two arXiv preprints carry that name and the correct identifier is arXiv:2606.30970. The claim of BEAM-substrate novelty in Section~\ref{sec:beam-novelty} is an absence-of-evidence observation. The reference implementation name is Trinity; the architecture name used in any downstream evaluation should be substrate-neutral.
...
\bibitem{croft2026eval}
A. Croft.
\newblock Authority-Bound Agentic Execution: Measuring Unauthorized Effect Under Adversarial Load.
\newblock 2026. \url{https://doi.org/10.5281/zenodo.21755869}
\newblock [Preprint. The companion evaluation of the architecture specified here. Concept DOI: it
resolves to the latest version. Every measured result this paper attributes to a separately
published evaluation is reported there.]
