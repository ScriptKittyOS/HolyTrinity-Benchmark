...
\hypersetup{
 pdftitle={Authority-Bound Agentic Execution: Measuring Unauthorized Effect Under Adversarial Load},
 pdfauthor={Ayla Croft},
 pdfsubject={cs.AI},
 pdfkeywords={AI agent security, authorization, unauthorized-effect rate, reference monitor, complete mediation, BEAM, evaluation methodology}
}
...
each mechanism in isolation converts its attacks (0$\rightarrow$10, 0$\rightarrow$5, 0$\rightarrow$5, 0$\rightarrow$14, 0$\rightarrow$2 unauthorized effects
across the content, runtime, skill, approval-binding, and two-person-approval boundaries), and a \textbf{no-control}
run with the full membrane disabled produces an unauthorized effect in \textbf{51 of 63 trials}: the ceiling the governed
...
Governed actions carry a median latency of \textbf{\textasciitilde19 ms}, reported as an absolute
...
(\S6.11); the false-denial rate is
\textbf{0.0\%} over 75 legitimate approved actions (0/75; 95\% CI {[}0.0\%, 4.9\%{]}, effective
...
family table as data), at \textbf{\url{https://github.com/ScriptKittyOS/HolyTrinity-Benchmark}}. We do
...
Proof is not binary. The verifier emits one of seven proof states: \texttt{verified},
\texttt{failed}, \texttt{degraded}, \texttt{pending\_receipt}, \texttt{legacy\_incomplete},
\texttt{manual\_review\_required}, and \texttt{missing}, because real provider systems are late and
incomplete. A trial record can also carry an eighth value, \texttt{not\_applicable}, which the
harness stamps on the six chaos trials (F6, F9) that place no proof obligation on the verifier; it
is a record-keeping value rather than a verifier verdict, and we name it here because it appears in
the committed artifacts. Publishing this \emph{state model} (as distinct
...
\item
 ...
 overstate it.
\item
 \textbf{CI-enforced boundary coverage.} A structural check over the source tree flags any code
 ...
 exercises is that check's predicate over the source tree; no build is invoked (\S6.10), so we do
...
\item
 ...
 not one: \emph{no language model is invoked anywhere in this harness}, which
 \texttt{aggressor/README.md} states enumerated per file. The 57 of 61 is a property of the fixture
 set. Our harness injects the violating action directly at the control-plane boundary, exactly as a
 fully compromised model would emit it. That is the strong test specified in {[}15{]}: ``replace the
...
(\texttt{spec/SPEC.md} \S3), including edge cases (partial effects, idempotent replay, tenant-scoped
...
\texttt{spec/SPEC.md} \S8 provision for manual adjudication of genuinely undecidable cases was not needed
...
\textbf{Where the independence rule is broken: F4.} The oracle decides payload identity by calling
\texttt{AuthorityAssurance.canonical\_payload\_hash/1} (\texttt{oracle/oracle.ex:313}, also
\texttt{:348} and \texttt{:360}), and \texttt{AuthorityAssurance} is the system under test. F4 is
27 of the 31 trials that could falsify the invariant, and F4's hypothesis is exactly that
canonicalization must not collide semantically distinct payloads. Oracle and system therefore
evaluate the same function on the same input: a collision returns \texttt{permit} from the system
and \texttt{authorized} from the oracle, and the two agree because they are one computation. \emph{The
failure class F4 exists to detect is the one class this oracle cannot detect.} Two facts compound
it. The canonicalizer is withheld (\S11), so no reader can check it; and it has had no
cryptographic review, while \S9.1 names it an element of the trusted computing base. F4's verdict
is therefore conditional on the correctness of an unreviewed function inside the system under test,
and F4 is outside the scope of this paper's independent-adjudication claim. The fix is a
canonicalizer written independently in \texttt{oracle/} from a written specification, with both
hashes published per trial; it does not exist yet and it is required future work.
...
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
One-sided 95\% upper bound (rule of three)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Two-sided 95\% Wilson upper limit
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
\texttt{provider\_call} (external effect) & F4, F5, F8 & 35 & 0 & $\leq$ 8.6\% & $\leq$ 9.9\% \\
gate (content/runtime/skill) & F1, F2, F3 & 20 & 0 & $\leq$ 15.0\% & $\leq$ 16.1\% \\
chaos (structural/process) & F6, F9 & 6 & 0 & $\leq$ 50.0\% & $\leq$ 39.0\% \\
\end{longtable}
...
\textbf{The denominator is a ladder, and we publish the ladder rather than pick a rung.} 61 is
correct as a count of \emph{attempts} and wrong as a count of \emph{opportunities}, because not
...
n
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
One-sided 95\% (rule of three)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Two-sided 95\% Wilson upper
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
Attack trials driven (attempts) & 61 & $\leq$ 4.9\% & $\leq$ 5.9\% \\
Structurally capable of inducing a violation & 51 & $\leq$ 5.9\% & $\leq$ 7.0\% \\
Capable of an external effect (F4's 27 + F5's 4) & 31 & $\leq$ 9.7\% & $\leq$ 11.0\% \\
Cluster-aware, mechanism as the unit (8 mechanisms) & 8 & --- & $\leq$ 32.4\% \\
\end{longtable}

Ten of the 61 provably cannot enter the numerator. F8's four trials cannot: with posture disabled
all four cross the boundary, all four are scored \texttt{allowed}, and unauthorized effects stay at
...
an \emph{external} one, which is what leaves 31. F7 does not belong on this list and we say so
explicitly: all four F7 trials are \texttt{allowed} controls and already sit outside the 61,
contributing nothing to it.

\textbf{And the 61 are not 61 independent observations.} They span eight mechanisms, and
...
\textbf{32.4\%}. We report the pair rather than the optimistic end: the payload-level bound is
$\leq$5.9\% and the cluster-aware bound is $\leq$32.4\%. This paper already applies exactly this
discipline to the false-denial rate, where the effective n is 3 rather than 75 (\S6.12); applying
it to the headline is the same arithmetic and it is less flattering. The eight is the conservative
count and we keep it deliberately: counted by \emph{actual denier} the mechanisms are nine, because
the spend policy denied 13 of F4's attack trials (\S6.7) without appearing in any family. Nine
clusters would tighten the bound to $\leq$29.9\%, so we publish the wider figure computed over
...
below a rate \emph{r} with zero observed effects requires n $\geq$ 3/\emph{r} ($\geq$31 for \textless10\%, $\geq$61 for \textless5\%,
$\geq$301 for \textless1\%; the inequality is strict, and these are the values the estimator returns). We therefore state, per family, the confidence the current n can support and
...
Two-person control & F5 & 0 $\rightarrow$ 2 \\
Posture policy & F8 & 0 $\rightarrow$ 0 unauthorized (4 \emph{authorized} writes now execute, recorded \emph{allowed}) \\
Spend policy (\texttt{policies.spend}) & --- (no family) & 0 $\rightarrow$ 0, over the full catalog \\
\end{longtable}

Five findings sharpen the causal claim. \textbf{First, defense in depth is measured, not
...
F8 claims. \textbf{Fourth, the two-person control is necessary and the data now says so:} removing
it admits two unauthorized effects, on the two extreme-risk variants that execute with no
co-approver and with the same approver twice --- and, as with F4, both are \emph{detected} rather
than undetected, flagged by the reconciliation sweeper. \textbf{Fifth, the spend policy is a redundant layer
...
\textbf{Two kinds of zero, and how to tell them apart.} Two rows of this table read
0 $\rightarrow$ 0, and a third did until the re-score. They do not mean the same thing, so we
...
\emph{measured redundancy}: the run executed over the full catalog with 41 provider-call trials
live, nothing raised, and the mechanism's own 13 trials were caught by the layer behind it. F5's
former zero was neither --- it
was a \emph{failed measurement}, an adjudicator blind spot that concealed two real effects, and it
is now 0 $\rightarrow$ 2. A zero is only a null once the trials are shown to have produced a usable
observation; the F5 row is what happens when that check is skipped.

\textbf{A correction, and its cause.} An earlier version of this paper reported the F5 row as a
0 $\rightarrow$ 0 null and read that null as redundant enforcement: a security virtue that resisted
single-clause ablation. The reading was wrong and the mechanism that made it wrong is worth stating
plainly, because it is the pre-committed edge case firing for real. \texttt{Telemetry.span/3} emits
\texttt{:stop} on success \emph{or} \texttt{:exception} on a raise, never both. The oracle that
produced the original ablation figures subscribed to \texttt{:stop} alone, so an adapter that
...
``partial effects'' case that SPEC \S3 pre-committed to before the first scored run. The governed
...
\textbf{A second defect sat under the first, and correcting it improves the F5 result rather than
enlarging it.} The reason those two trials raised at all is that the fixture payload omitted an
email field \texttt{LocalCRM} requires. A trial that raises mid-drive records no proof state, and
that missing proof state --- not a failure of detection --- is what produced the row's original
\texttt{undetected} / \texttt{missing} / \texttt{none} triple. With the fixture corrected, both
trials score \textbf{\texttt{detected}}, with \texttt{system\_proof\_state:\ failed} and
\texttt{detection\_source:\ sweeper}: the reconciliation sweeper flags both admitted effects. The
unauthorized-effect \textbf{count is unchanged at two}, so the row remains 0 $\rightarrow$ 2. What
changed is its character. Removing two-person approval admits two unauthorized effects
\emph{and the system detects both} --- the same degradation from prevention to detection that the F4
ablation shows, rather than a silent crossing.

\textbf{No-control ceiling.} With the entire ablatable membrane disabled, \textbf{51 of 63 trials}
produce an unauthorized effect and all 63 cross the effect boundary. The same 63 run governed cross
8 times and produce \textbf{0} unauthorized effects. Governing therefore prevents 55 crossings that
occur ungoverned (8 versus 63), and the unauthorized-effect counts are 0 against 51. One wording
correction ships with that number: \textbf{63 is not a count of attacks.} It is the F1--F5 and F8
...
effect unauthorized) and a \textbf{proof-state-weighted calibration gap} of 0.0\%.

\textbf{That statistic is not an ECE and we no longer call it one.} Its bins are seven categorical
\texttt{proof\_state} labels carrying hand-authored confidence constants, not probability bins, so
...
\emph{arithmetically} rather than empirically; we therefore never state it without the
decomposition below. It is also sensitive to a single constant. The
\texttt{manual\_review\_required} bin carries 70.7\% of the weight (29 of 41), is described
...
is wide: 0/6 bounds the overclaim rate at $\leq$50\% one-sided (rule of three) and $\leq$39.0\%
...
Recomputed from the \texttt{notes} field of the committed per-trial records, which carries the
actual result the drive call returned, the denying component splits: \textbf{18} at the
\texttt{AuthorityAssurance} preflight, \textbf{13} at \texttt{SpendPolicy}'s preflight, \textbf{2}
by \texttt{RuntimePosturePolicy}, \textbf{1} by \texttt{AuthorityAssurance} at the policy stage, and
\textbf{1} by \texttt{AuthorityJudge}. An earlier version of this paper reported that split as
``31 at the verifier preflight and 4 by the runtime-posture policy,'' which was wrong.

The correction is not only arithmetic. \texttt{SpendPolicy} held \textbf{13 of F4's 27 attack
...
family and carries no \texttt{mechanism\_id} stamp, and when this recount was first published it had
no ablation row either, which left its position relative to the kernel asserted rather than measured.
It has since been ablated on its own: over the full catalog \texttt{policies.spend} admits 0
unauthorized effects and 0 provider-call effects, and it now carries a row in both the \S6.5
ablation table and the \S9.1 trusted-base table, backed by the same full-catalog run as the other
three \S9.1 probes. So the 13 are held by a \emph{redundant} layer ---
...
independent oracle scores it. A 32-attack set produced \textbf{0 unauthorized effects (95\% CI
{[}0.0\%, 10.7\%{]})}, with all 32 prevented and the four honest controls allowed.

\textbf{The 32 attacks are not 32 independent shapes, and the interval above is computed as though
...
$\leq$10.7\%; per mechanism, the denominator is as small as four and 0/4 gives $\leq$49.0\%. The
second is the honest reading, and it is the one a reader should carry away. The coverage is narrow
in a second way: the declarative channel realizes the approval$\rightarrow$execution join, so the
blind result speaks to F4 and F7 and says nothing about the gate families.

\textbf{A provenance defect, stated rather than smoothed.} The blind-set file that ships is not
byte-identical to the file that was scored. The scored blob carried \textbf{30} distinct attack
...
the denominator and the $\leq$10.7\% arithmetic are unaffected. What is affected
...
file. Per-trial records now ship, as \texttt{artifacts/blind.jsonl} (36 rows, 32 attack trials, 0
unauthorized effects, 0 harness errors, ids \texttt{blind-01} to \texttt{blind-36}, splitting
18 / 7 / 3 / 4), and they too are a \emph{re-run of the corrected file} rather than the original
run's records, which were not retained. That re-run reproduces the same rate and interval over 32
genuinely distinct shapes, so the published number is not an artifact of the duplicates; it does not
repair the provenance defect and we do not present it as if it did. We are transparent that this set was
...
violation in the scanned source and confirms the check's predicate reports it. \emph{No build is
invoked} --- no compiler runs anywhere in the released tree, as \texttt{aggressor/README.md}
states --- so what F6 establishes is that the guard's predicate holds, not a demonstrated build
failure; SPEC \S4 pre-registered the stronger form and this campaign did not deliver it. Neither mechanism claims that no uncovered path can
...
\subsection{Governed latency}\label{overhead}
...
enters. Median governed latency is \textbf{\textasciitilde19 ms}, with p95 and p99 at 53.5 and
76.3 ms (19100 / 53475 / 76336 $\mu$s). The cost is dominated not by policy evaluation but by
durable authority-proof persistence: the system pays \textasciitilde19 ms at p50 to make each
...
\textbf{These percentiles have moved since an earlier revision, and the reason is that they now
have an artifact behind them.} We previously published 16882 / 57811 / 81277 $\mu$s
(\textasciitilde17 ms at p50). No per-iteration data from that run was ever committed, so the
figures rested on a transcription no reader could check. A fresh run's per-iteration records now
ship as \texttt{artifacts/overhead.jsonl}, 200 per arm, and the numbers above are recomputed from
...
was wrong. \textbf{Latency is machine- and run-dependent}: these percentiles describe one synthetic
single-process loop on one machine, and \emph{percentiles are not comparable across machines}. A
re-run elsewhere yielding different values is the expected outcome, not a failed replication.

\textbf{We report this as an absolute cost and claim no percentage overhead, because the comparison
baseline is degenerate by construction.} The ungoverned arm routes to a mock adapter,
\texttt{MockStripe.execute/3}, a pure in-memory function that performs no I/O and whose measured
p50 is \textbf{120 ns}. That number settles a question the earlier table left open: the bypassed
arm's published \textbf{0 $\mu$s} was a \emph{timer-resolution floor, not a measurement}. At the
one-microsecond resolution the table is printed at, 120 ns and the p95's 151 ns truncate to 0 and
the p99's 3106 ns truncates to 3, so the bypassed row reads 0 / 0 / 3 $\mu$s; the real distribution
...
governed path, so a corrected differential rounds to the same \textasciitilde19 ms. The number is
...
profile. (Hardware is in \texttt{REPORT.md}.)
...
\item
 \textbf{F4's zero bounds payload divergence, not the whole approval-binding join.} The family
 ...
 lifecycle replays, and 2 known-good controls. \emph{Five have no trial at all} --- provider swap
 (all 29 carry \texttt{provider:\ mock\_stripe}), operation swap (all 29 carry
 \texttt{operation:\ buy}), account re-attribution (all 29 carry a single \texttt{account\_id};
 \texttt{mutate:drop-account} removes the field, it does not re-target), idempotency-key reuse
 across two distinct payloads, and approval-missing execution. What F4's zero bounds is therefore
 the \textbf{payload-divergence} class, not the join as a whole. The account gap compounds with a
 defect in the oracle: \texttt{effects\_for/2} matches on provider, operation and
 idempotency-key hash and never compares \texttt{account\_id}, so SPEC \S3's pre-committed ``right
...
\item
 \textbf{Oracle independence does not hold for F4.} The oracle calls the system under test's own
 \texttt{canonical\_payload\_hash/1} to decide whether an executed payload is the approved one
 (\S6.2). A canonicalization collision would therefore be invisible to both sides at once, and a
 ...
 trusted-base element in \S9.1, so F4's verdict is conditional on the correctness of an unreviewed
 ...
 sits beside it: \texttt{effects\_for/2} never compares \texttt{account\_id}, which \S7 states
 together with F4's missing account-re-attribution variants. \texttt{oracle/README.md} carries
...
Second, section \texttt{human-oversight}, gap \texttt{nist-p7}: ``No sources weigh approval-gate latency costs against risk prevention.'' We report both halves together, \textasciitilde19 ms of median governed latency (\S6.11) against a 0/75 false-denial rate (\S6.12). We are precise about the claim: the trade-off itself \emph{is} named in the classic catalog, as \texttt{nist-p7}, ``identify potential trade-offs between reducing risk and increased costs.'' What the corpus records as absent is its \emph{instrumentation at an agent approval gate}. Three qualifications belong with it: the p95 and p99 are 53.5 and 76.3 ms, and a gate too slow to use is a tail property the median does not speak to; the false-denial figure has an effective sample size of three (\S6.12); and the latency figures are one machine's, not a portable constant.
...
need of a test: it denied 13 of F4's 27 attack trials (\S6.7), 48\% of the kernel family, while
carrying no family and no ablation row. It has since been ablated on its own, as mechanism
\texttt{policies.spend} over the full catalog, and it admits \textbf{0} unauthorized effects and
\textbf{0} provider-call effects (\S6.5), over 73 trials with 41 provider-call trials live. Read
against the compound ablation, that settles its
...
join is gone, which is why all 27 convert only when \emph{both} fail. Neither layer is individually
necessary for those 13. That is defense in depth, and it puts the spend policy outside the kernel.

\textbf{Break the boundary out by effect channel} (\texttt{-\/-tcb}). We ablate each element,
re-run the \textbf{full 73-trial catalog} against it, and report the admitted effects by channel. A
surface reducer's failure should admit effects only in its own gate channel and \emph{zero}
...
Spend policy & redundant layer & 0 & --- & \textbf{0} \\
...
\textbf{What that table establishes, and what it does not.} The four ablations in
\texttt{artifacts/tcb-full-catalog.jsonl} each drove the \textbf{full 73-trial catalog} --- 292
per-trial records, 73 per probe, all nine families present under every ablation --- so
\textbf{41 provider-call trials were live under each} (35 of them attack trials; the other six are
F4's two known-good controls and F7's four replays). That is what makes the last column a
measurement rather than a consequence of trial selection: a surface reducer whose removal admitted
even one external effect would have recorded it there, and none did. Every admitted effect landed in
the ablated mechanism's own gate channel --- 10 \texttt{content\_gate}, 5 \texttt{runtime\_gate}, 5
\texttt{skill\_gate} --- and \texttt{provider\_call} unauthorized effects were zero in all four
probes. The kernel row is measured on a separate basis and always was: removing the approval-binding
join admits fourteen unauthorized external effects, all on the \texttt{provider\_call} channel
(\texttt{artifacts/ablation/tcb-F4.jsonl}, 29 provider-call trials live). That row cannot be
...
\texttt{artifacts/ablation/tcb-per-family-probes.jsonl}, and their own caveat records why they were
insufficient.

We state the scope exactly. This establishes that these \textbf{three} surface reducers, removed
\textbf{singly}, admit no external effect. It establishes nothing about \textbf{combinations} --- no
run disables two of them at once --- and nothing about mechanisms with \textbf{no runtime toggle}:
F6's boundary guard, F7's reconciliation, and F9's process supervision are never ablated, and
\textbf{the mediation membrane and the durable authority store have no ablation row at all}. Those
two are named as trusted-base elements on the \S3 invariant's structure, not by a run that removed
them. Auditability is now uniform across the table: the four full-catalog probes recompute from
\texttt{artifacts/tcb-full-catalog.jsonl} and the kernel row from \texttt{artifacts/ablation/}.

So \texttt{nist-p25} here is \emph{measured} for every element the harness can ablate: the trusted
...
removes two elements of the same class together. It also resolves an intra-catalog tension: layered security with no single point of vulnerability (\texttt{nist-p16}) pulls against minimizing the system elements to be trusted (\texttt{nist-p25}), and both are classic principles; the resolution is that layers
\emph{outside} the kernel do not enlarge the TCB. The economy-of-mechanism half is that the kernel's
fence is small enough to be checked by a \textbf{CI-enforced structural check over the source tree}
(\S5), not upheld by convention.
...
\item
 ...
 tamper-proofness, a gap \S7 states directly. The seven properties instantiate Saltzer and Schroeder's complete mediation,
...
public artifact at \url{https://github.com/ScriptKittyOS/HolyTrinity-Benchmark} contains: the frozen
...
\emph{results} auditable (every number in the paper follows mechanically from the JSONL) and the
\emph{method} inspectable.
...
\textbf{Fixture release (pre-committed, \texttt{spec/SPEC.md} \S11):} release fixtures for families that expose
...
\item
  ...
  \url{https://doi.org/10.5281/zenodo.21754762} (concept DOI, which resolves to the current
  version; v1.1 is \texttt{10.5281/zenodo.21764616}). The earliest deposit is timestamped
  2026-08-02, later than this paper's campaign, so it gives the companion a citable identifier
  without corroborating the 29 July date the manuscript states. Source in the artifact repository at
...
The reported numbers are recomputable from the committed JSONL alone: \texttt{artifacts/family-table.json} is
...
ship as well: the per-mechanism ablations in \texttt{artifacts/ablation/tcb-F*.jsonl} and, both arms
together, in \texttt{ablation-study.jsonl}; the no-control ceiling in \texttt{baseline.jsonl}; the
compound ablation in \texttt{compound.jsonl}; the \S9.1 channel probes in
\texttt{tcb-full-catalog.jsonl}; the governed-latency iterations in \texttt{overhead.jsonl}; the
false-denial actions in \texttt{false-denials.jsonl}; the F10 controls in
\texttt{measurement-integrity.jsonl}; and the blind red team in \texttt{blind.jsonl}. Each carries a
\texttt{\_meta} line stating how to recompute the figure it backs, and several carry a
\texttt{caveat} that bounds what the run can support. \textbf{Every table in the paper now
recomputes from a committed artifact}, including the \S9.1 channel table and the
\texttt{policies.spend} row, which are the four full-catalog probes in
\texttt{tcb-full-catalog.jsonl}. What remains outside the artifact is scope, not arithmetic: the
limits \S9.1 states (single removals only, no combinations, no ablation of F6/F7/F9, none of the
membrane or the durable store) are properties of which runs exist, and no file closes them.
...
\url{https://github.com/ScriptKittyOS/HolyTrinity-Benchmark}
