...
`baseline@29336393` (no `+dirty` marker). Both committed artifacts in `artifacts/` are clean-tree runs (no `+dirty`); the v1 file carries an
*ablation* stamp (`ablate:hermes.skill_auditor.exfil_detector@00568aca`) rather than the baseline one; all 73 trials — F6/F9 included — now carry the real config
...
load-bearing (0→10/5/5/14 when removed); (3) the no-control baseline shows 47/63 attacks
...
```text
  denominator                                  n    95% CI upper   rule-of-3
...
`mechanism_id`; across the 61 attack trials there are **eight distinct mechanisms** —
approval-binding 27, content firewall 10, runtime sentinel 5, skill auditor 5, two-person 4,
runtime posture 4, boundary guard 3, runtime boss 3. The ablation above shows the within-mechanism
correlation is essentially 1: removing one mechanism converts *all* of its trials at once (10/10,
5/5, 5/5, 4/4). When trials cluster that hard, the effective sample size is the number of
...
effect (the 2 F4 controls + 4 F7 replays); of 41 provider-call trials, **35 were denied before the
adapter ran** — **31** at the verifier preflight and **4** by the runtime-posture policy (the F8
family). Note that `detection_source` on a *prevented* trial records the family's **pre-registered
...
the independence rule forbids. The per-trial `notes` field carries the actual denial returned by
the drive call. So "0 unauthorized effects" is measured through a
...
not in the governed run.** The ablation campaigns produced and *observed* 10, 5, 5,
and 14 effects (F1/F2/F3/F4) and the no-control campaign 47; the Oracle's effect log demonstrably fires when effects
